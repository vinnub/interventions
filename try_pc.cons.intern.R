try_pc.cons.intern <- function (sk, suffStat, indepTest, alpha, version.unf = c(NA, 
                                                          NA), maj.rule = FALSE, verbose = FALSE) 
{
  ...
  if (sum(g) > 0) {
    ...
    if ((m <- nrow(tripleMatrix)) > 0) {
      ...
      for (i in seq_len(nrow(tripleMatrix))) {
        ...
        r.abc <- checkTriple(a, b, c, nbrsA, nbrsC, sk@sepset[[a]][[c]], 
